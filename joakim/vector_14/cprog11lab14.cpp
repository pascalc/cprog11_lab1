...
int main(int argc, char **argv) {
    ...
    while(cin >> command) {
        ...
        if(command == "new" ) {
            ...
            cout << idx << " copied, size=" << vectors_s.size() << "\n";
        } else if(command == "delete") {
            const size_t idx = read_idx_or_die();
            delete vectors[idx];
            vectors[idx] = 0;
            vectors_s[idx] = 0;
            cout << idx << " deleted, size=" << vectors_s.size() << "\n";            
            ...
            *vectors[to] = *const_cast<const Vector*>(vectors[from]);
            ...
            const size_t idx2 = read_size_or_die();
            try {
                cout << (*const_cast<const Vector*>(vectors[idx]))[idx2] << "\n";
            } catch (std::out_of_range oor) {
                cout << "std::out_of_range" << "\n";
            }
        } else if(command == "inc") {
